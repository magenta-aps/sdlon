...
class ChangeAtSD:
    ...
    def apply_NY_logic(self, org_unit, employment_id, validity, person_uuid) -> str:
        ...
        # since we cannot get SD department parents *with a validity* back in time
        # via the SD API. It is only possible to get an SD department parent on
        # a specific date. Calling fix_department with a date argument prior to
        # today's date can therefore result in an incorrect parent
        # (see more details on this Redmine issue
